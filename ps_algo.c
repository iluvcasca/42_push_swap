...
/*   Updated: 2024/04/01 15:16:27 by kgriset          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
#include "push_swap.h"
...
static void	compute_mov2(t_vars *vars, t_mov_vars *mov_vars)
{
	...
	while (mov_vars->j < vars->cdl_list_a->total
		&& abs(mov_vars->array_a[mov_vars->j]
			- mov_vars->array_b[mov_vars->i]) != mov_vars->insert)
		++(mov_vars->j);
	if (mov_vars->array_a[mov_vars->j] - mov_vars->array_b[mov_vars->i] < 0
		&& mov_vars->j + 1 < vars->cdl_list_a->total)
		++(mov_vars->j);
	else if (mov_vars->array_a[mov_vars->j] - mov_vars->array_b[mov_vars->i] < 0
		&& mov_vars->j + 1 == vars->cdl_list_a->total)
		mov_vars->j = 0;
}
...
{
	if (mov_vars->mov_a[mov_vars->j] < 0 && mov_vars->mov_b[mov_vars->i] < 0)
		mov_vars->sum = max_size((size_t){mov_vars->mov_a[mov_vars->j] * -1},
				(size_t){mov_vars->mov_b[mov_vars->i] * -1});
		...
		mov_vars->sum = max_size((size_t)mov_vars->mov_a[mov_vars->j],
				(size_t)mov_vars->mov_b[mov_vars->i]);
