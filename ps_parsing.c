...
/*   Updated: 2024/04/01 15:25:32 by kgriset          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	parse_string(char *string, t_circular_db_ll *cdl_list, int *status)
{
	...
	while (inputs[i])
	{
		...
		if (!(*status == SUCCESS && add_int2node(cdl_list, cdl_node,
					vars.value)))
			return (free(vars.temp), free_split(inputs),
				cdl_free_list(cdl_list), ERROR);
		free(vars.temp);
		i++;
	}
...
int	parse_arguments(int argc, char **argv, t_circular_db_ll *cdl_list,
		int *status)
{
	int					value;
	size_t				i;
...
int	add_int2node(t_circular_db_ll *cdl_list, t_double_link_node *cdl_node,
		int value)
{
